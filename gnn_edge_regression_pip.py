...
from model_pip import EdgeWeightPredictor
...
def main(methods, transform_method):
    ...
    for method in methods:
        ...
        if not not_continue:
            ...
            model = EdgeWeightPredictor(num_features, 1)
            ...
            # Ploter
            plot = Plot()
            plot.graficar_loss_no_cutoff(train_loss, val_loss, name, method, 'edge_regression', num_epochs, transform_method, 'gnn')
            # Guardar modelo
    ...
    tabla.to_csv('tables_gnn/' + transform_method + '/pip_edge_regression_table' + str(num_epochs) + '.csv', index=False)
