...
def main(methods, transform_method):
    ...
    for method in methods:
        ...
        if not not_continue:
            ...
            model = MLP(num_features, num_features//2, num_features//2)
            ...
            neg_train_edges = global_uniform_negative_sampling(sub_g, sub_g.num_edges())
            ...
            neg_val_edges = global_uniform_negative_sampling(sub_g_val, sub_g_val.num_edges())
            ...
            # Ploter
            plot = Plot()
            plot.graficar_loss(train_loss, val_loss, cutoff, name, method, 'linkpred', num_epochs, transform_method, 'mlp')
            plot.graficar_accuracy(train_accuracys, val_accuracys, cutoff, name, method, 'linkpred', num_epochs, transform_method, 'mlp')
            # Guardar modelo
    ...
    tabla.to_csv('tables_mlp/' + transform_method + '/pip_linkpred_table' + str(num_epochs) + '.csv', index=False)
