...
from models_pip import EdgeWeightPredictor
import torch.nn.functional as F


device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

def main(feature, transform, epochs, type_model, test_split, hidden_layers, database):
    Dataframe = pd.read_csv('../' + database + '/edge_index.csv', na_filter = False, dtype={'edge_1': int, 'edge_2': int})
    edge_1 = Dataframe['edge_1'].values
    edge_2 = Dataframe['edge_2'].values
    Dataframe = pd.read_csv('../' + database + '/weights.csv', na_filter = False, dtype={'weights': float})
    weights = Dataframe['weights'].values
    if transform is None:
        transform = 'no_transform'
        Dataframe = pd.read_csv('../' + database + '/no_transform/' + feature + '/nodes_features_.csv', na_filter = False, sep='|')
    elif transform == 'pca':
        Dataframe = pd.read_csv('../' + database + '/pca/' + feature + '/nodes_features_.csv', na_filter = False, sep='|')
    else:
        Dataframe = pd.read_csv('../' + database + '/kernel_pca/' + feature + '/nodes_features_.csv', na_filter = False, sep='|')
    ...
    print('Resultados guardados en resultados/' + database + '_' + type_model + '_edge_regressor_epochs_' + str(epochs) + '_test_' + str(test_split) + '_hidden_layers_' + str(hidden_layers) + '.csv')
    ruta = 'resultados/'
    if not os.path.exists(ruta):
        os.mkdir(ruta)
    table.to_csv('resultados/' + database + '_' + type_model + '_edge_regressor_epochs_' + str(epochs) + '_test_' + str(test_split) + '_hidden_layers_' + str(hidden_layers) + '.csv', index=False)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser = argparse.ArgumentParser(add_help=False)
    parser = argparse.ArgumentParser(usage="(example) gnn_edge_regressor_pip.py --features bepler --transform(optional) pca --epochs 200 --test 0.3")
    ...
    databases = ['pip_database_data_modelos', 'proximate_dg_data_modelos', 'proximate_kd_data_modelos',
                 'proximate_kon_data_modelos', 'skempi_affinity_data_modelos', 'skempi_koff_data_modelos', 'skempi_kon_data_modelos']

    for database in databases:
        if args.transform is None:
            main(args.features, None, args.epochs, 'gnn', args.test, args.hidden, database)
        else:
            main(args.features, args.transform, args.epochs, 'gnn', args.test, args.hidden, database)
