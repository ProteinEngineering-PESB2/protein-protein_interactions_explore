...
def main(methods, transform_method):
    ...
    for method in methods:
        ...
        model = Net(in_channels=num_features, hidden_channels=round(num_features/2), out_channels=round(num_features/2))
        ...
        for epoch in range(num_epochs):
            ...
            # median = torch.quantile(scaled_values, 0.5)
        ...
        with torch.no_grad():
            ...
            loss_binary = F.binary_cross_entropy_with_logits(test_binary, test_edges_label.float(), reduction='mean')
            test_accuracy = (test_binary == test_edges_label).float().mean().item()
            # print('------------------TEST--------------------')
        ...
        # Ploter
        plot = Plot()
        plot.graficar_loss(train_loss, val_loss, cutoff, name, method, 'linkpred', num_epochs, transform_method)
        plot.graficar_accuracy(train_accuracys, val_accuracys, cutoff, name, method, 'linkpred', num_epochs, transform_method)
        # Guardar modelo
    ...
    tabla.to_csv('tables/' + transform_method + '/pip_linkpred_table' + str(num_epochs) + '.csv', index=False)
